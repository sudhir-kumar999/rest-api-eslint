  ...
  try {
    ...
    fs.readFile("blog.json","utf-8",(err,data)=>{
      if(!err && data){
        ...
        const filterData:updateBlog[]=readData.filter((ele)=>ele.author_id==author_id);
        ...
        const filterBlog:updateBlog[]=filterData.filter((ele)=>ele.post_id==blog_id);
  ...
  try {
    ...
    fs.readFile("blog.json","utf-8",(err,data)=>{
      if(!err && data){
        ...
        const filterPost=filterData.findIndex((ele)=>ele.post_id==blog_id);
        if(filterPost==-1){
          return res.status(404).json({
            success:false,
        ...
        const newData:blogType[]=readData.filter((ele)=>ele.post_id!=blog_id);
